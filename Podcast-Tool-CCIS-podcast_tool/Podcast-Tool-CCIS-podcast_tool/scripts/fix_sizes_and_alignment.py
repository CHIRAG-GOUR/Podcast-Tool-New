...
# Replace the switch block that hardcodes font sizes with relative scales based on clip.style.fontSize
ui_replacement = """                        {(() => {
...
content = re.sub(r'                        \{\(\(\) => \{\s*const preset = clip\.style\?\.preset \|\| \'hormozi\';.*?if \(preset === \'cinematic\'\) \{.*?\}', ui_replacement, content, flags=re.DOTALL)
with open(studio_path, 'w', encoding='utf-8') as f:
    f.write(content)

# 2. Update export/route.ts
export_path = r'e:\1. Skillizee\Podcast Tool\src\app\api\video\export\route.ts'
with open(export_path, 'r', encoding='utf-8') as f:
    e_content = f.read()

# Replace generateAssFile signature and logic
...
e_content = e_content.replace(
    "const assContent = generateAssFile(captions, targetWidth, targetHeight, style.preset || 'hormozi');",
    "const assContent = generateAssFile(captions, targetWidth, targetHeight, style.preset || 'hormozi', style.fontSize || 48);"
)

with open(export_path, 'w', encoding='utf-8') as f:
    f.write(e_content)

print("Font sizes and alignments strictly corrected.")
